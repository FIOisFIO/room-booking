class MockApi {
    ...
    getRooms(tower, floor) {
     return fakeApi(new Array(10).fill().map((elem, i) => ({name: createRoomNumber(tower, floor, i+1), id: createRoomId(tower, floor, i+1)})))
    }
    getFloors() {
        return fakeApi(new Array(25).fill().map((elem, i) => ({name: `Этаж ${i+3}`, id: i+3})))
    }
